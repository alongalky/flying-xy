...
def find_transition_noise_by_pol_inflexion(pols):
    ...
    ax1.set_title(r'D = {:.2f}, $\rho$={:.3f}'.format(noise, density), fontsize=14, fontweight='bold')
    ax1.set_xlabel('$A$', fontsize=14, fontweight='bold')
    ax1.set_ylabel(r'$<\Pi>$', fontsize=14, fontweight='bold')
    pols = sorted(pols, key=lambda x: x.input.align)
    x_dat = np.array([x.input.align for x in pols])
    ...
    if trans == None:
        return None
    return transitionPoint(noise=noise, density=density, align=c.get_trans(), pols=pols, method=transitionMethods.polInflection,
        const_align=True)
...
def find_transition_noise_by_pol(pols):
    ...
    avg_align = np.dot(x_dat,y_dat)/np.sum(y_dat)
    ###
    #fig = plt.figure()
    #ax1 = fig.add_subplot(111)
    ...
    density = getDensityInput(pols[0].input)
    noise = pols[0].input.noise
    return transitionPoint(noise=noise, density=density, align=avg_align, pols=pols, method=transitionMethods.polFluctuation,
        const_align=True)
...
if __name__ == '__main__':
    ...
    print resultsDir
    pickleFilename = 'density_align_trans.pickle'
    ...
    ax1.set_xlabel('$A$', fontsize=14, fontweight='bold')
    ax1.set_ylabel(r'$\rho$', fontsize=14, fontweight='bold')
