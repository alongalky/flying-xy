...
if __name__ == '__main__':
    ...
    for output_dir in os.listdir(resultsDir):
        ...
        if 'output' in output_dir and os.path.isdir(full_dir):
            try:
                input = read_input_from_dir(full_dir)
                if input.align != args.align:
                    #print input.align, ' is not ', args.align 
                    continue
            except:
                continue
            #if getDensityInput(input) > 10.1:
            #    continue
            shade = calc_pol_var(full_dir)
    ...
    title = '{} {}, A={}'.format(txt, model, input.align)
    if not args.minimal:
    ...
    plot_transition_curves(ax1, 'noise_density_trans.pickle', align=args.align)
    if isfile('density_noise_trans.pickle'):
        print 'yay'
        plot_transition_curves(ax1, 'density_noise_trans.pickle', align=args.align, sortbyrho=True)
    plt.show()